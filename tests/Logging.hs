...
instance Arbitrary UTCTime where
  arbitrary = do
    randomYear <- choose (1900, 2100) :: Gen Integer
    ...
    LogRow
      ...
      <*> ( Aeson.object
              <$> listOf
                ( (,)
                    <$> ( arbitrary
                            `suchThat` (`notElem` ["type", "time", "log_level"])
                        )
...
propLogRowJsonRoundtrip :: SpecWith ()
propLogRowJsonRoundtrip =
  prop "round trips to json" $ \logRow ->
    Aeson.decode (Aeson.encode logRow) `shouldBe` Just (logRow :: LogRow)
...
logTest1 =
  LogTest
    { logTestTime = Just log1TestTime,
      logTestLevel = LevelInfo,
      logTestType = "test",
      logTestPayload = Aeson.object ["foo" Aeson..= False, "bar" Aeson..= True]
    }

-- caseLlog = withDB $ \pool -> runApp readCommitted def pool () $ do
--   logEvent LogTest
--            { logTestTime    = Nothing
--            , logTestLevel   = LevelInfo
