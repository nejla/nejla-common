...
mkLogsstashConfPaths pwd name =
  ( pwd <> "/tests/logstash/conf.d/" <> name
  , "/etc/logstash/conf.d/"<> name
  )


confVolume :: Text -> Text -> [Text]
confVolume pwd name = ["-v", let (lPath, dPath ) = mkLogsstashConfPaths pwd name
                    in lPath <> ":" <> dPath
                  ]
...
checkConfFiles = do
  ...
  forM_ logstashConfNames $ \name -> do
    ...
    unlessM (test_f $ fromText path) . terror $ "File" <> path <> " does not exist"
...
runLogstash f = shelly . silently $ do
  ...
  stripped <- case Text.stripPrefix "Sending logstash logs to /dev/null.\n" out of
                Nothing ->
                  terror "Expected output prefix \
                         \ \"Sending logstash logs to /dev/null.\\n\""
                Just s -> return s
  return $ splitLines stripped
  where
    splitLines lines =
      case Text.breakOn "}{" lines of
        (rest, "") -> [rest]
...
case_logstash = do
  ...
  case rows of
    [ Just (Aeson.Object row1)
     ,Just (Aeson.Object row2)
     ] -> do
      withRow row1 $ do
  ...
  where
    shouldDecodeTo x y = ReaderT $ \o ->
      case HMap.lookup x o of
        Nothing -> assertFailure $ "could not find element" <> show x
        Just v -> case Aeson.fromJSON v of
                    Aeson.Error e -> assertFailure $ "Could not decode "
                                                     <> show x
                                                     <> " because " <> e
