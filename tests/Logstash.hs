...
import Shelly
import Test.Hspec
import NejlaCommon.Test (failure)
...
runLogstash f = shelly . silently $ do
  ...
  stripped <-
    case Text.stripPrefix "Sending logstash logs to /dev/null.\n" out of
      Nothing ->
        terror
          "Expected output prefix \"Sending logstash logs to /dev/null.\\n\""
      Just s -> return s
...
spec :: Spec
spec = describe "logstash" $ do
  it "stores in logstash" $ do
    res <- runLogstash $ do
      logEvent logTest1
    ...
    case rows of
      ...
      _ ->
        failure $
          "Expected 1 row, instead got "
            <> unlines (Text.unpack <$> res)
  where
    shouldDecodeTo x y = ReaderT $ \o -> case HMap.lookup x o of
      Nothing -> failure $ "could not find element" <> show x
      Just v -> case Aeson.fromJSON v of
        Aeson.Error e ->
          failure $ "Could not decode " <> show x <> " because " <> e
