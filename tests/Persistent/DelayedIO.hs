...
case_delayedIO_runs_after = do
  countRef <- newIORef 0
  withDB $ \pool -> runApp readCommitted (sqlConf 3) pool () $ delayIO $
    inc countRef
...
case_delayedIO_runs_not_on_error = withRestarts 1 $ \restart -> do
  countRef <- newIORef 0
  Ex.handle (\(e :: PersistError) -> return ()) $ withDB $
    \pool -> runApp readCommitted (sqlConf 0) pool () $ do
      delayIO $ inc countRef
      restart
  count <- readIORef countRef
  count `shouldBe` 0
...
case_delayedIO_runs_multiple_in_order = withRestarts 1 $ \restart -> do
  ref <- newIORef []
  Ex.handle (\(e :: PersistError) -> return ()) $ withDB $
    \pool -> runApp readCommitted (sqlConf 3) pool () $ do
      delayIO $ push 1 ref
      delayIO $ push 2 ref
      delayIO $ push 3 ref
      restart
