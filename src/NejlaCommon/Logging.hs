...
instance ToJSON LogRow where
  toJSON lr =
    ...
    in case v of
           Object o -> Object $ o <> HMap.fromList commons
...
instance FromJSON LogRow where
  parseJSON = withObject "log row" $ \o -> do
    ...
    payload <- case mbPayload of
        Nothing ->
          parseJSON . Object $ o // "event" // "time" // "source" // "level"
        Just pl -> return pl
    return LogRow
           { logRowTime    = time
           , logRowEvent   = tp
...
fromLogFun
  :: (Loc -> LogSource -> LogLevel -> LogStr -> IO ()) -> LogRow -> IO ()
...
data LogEvent =
  LogEvent
  { logEventType    :: !Text -- ^ What sort of event this is (e.g. "login
                             -- failed").
...
-- IsLogEvent class
logEvent :: (MonadIO m, MonadLogger m, IsLogEvent a) => a -> m ()
...
toLogHeaders = fmap toHeader
  where
    toHeader (bsn, bsv) =
      LogHeader
      { logHeaderName  = tDecode $ CI.foldedCase bsn
...
-- very large
logHttpCalls :: (LogRow -> IO ()) -> Wai.Middleware
logHttpCalls logRequest app request' respond = do
  ...
  app request $ \response -> do
    ...
    let reqLog =
          ...
          , requestLogHeaders = toLogHeaders $ Wai.requestHeaders request
          , requestLogRequestBody = bst <$> reqBody
          , requestLogResponseCode =
              HTTP.statusCode $ Wai.responseStatus response
          , requestLogResponseHeaders =
              toLogHeaders $ Wai.responseHeaders response
          , requestLogResponseBody = body
          , requestLogIP = bst
              <$> List.lookup "X-Real-IP" (Wai.requestHeaders request)
          }
  ...
  where
    getBody nextChunk acc = do
      chunk <- nextChunk
      if BS.null chunk then return acc else getBody nextChunk (acc <> chunk)
    ...
    responseToText resp = do
      ...
      case Wai.responseToStream resp of
          (_, _, f) -> f $ \sb ->
            sb (\chunk -> modifyIORef ref (chunk :)) (return ())
      chunks <- List.reverse <$> readIORef ref
      let txt :: Text
          txt =
            Text.decodeUtf8With Text.lenientDecode . BSL.toStrict
            . BS.toLazyByteString $ mconcat chunks
      return $ Just txt
...
logOnException logFunction =
  Warp.setOnException $ \mbReq (Ex.SomeException e) -> do
    now <- getCurrentTime
    let (method, path) = case mbReq of
            Nothing -> ("N/A", "server")
            Just req ->
              ( Text.decodeUtf8With Text.lenientDecode (Wai.requestMethod req)
              , Text.decodeUtf8With Text.lenientDecode (Wai.rawPathInfo req)
              )
    let evt =
          ExceptionEvent
