...
setEmailConf
  :: (MonadIO m, MonadLogger m) => Conf.Config -> m (Maybe EmailConfig)
setEmailConf conf = getConfMaybe "EMAIL_FROM" "email.from" conf >>= \case
    ...
    Just fromAddress -> do
      ...
      emailConfigPort <- getConf' "EMAIL_PORT" "email.port" (Right 25) conf
      emailConfigUser
        <- getConf "EMAIL_USER" "email.user" (Left "email user") conf
      emailConfigPassword <- getConf "EMAIL_PASSWORD"
                                     "email.password"
                                     (Left "email password")
                                     conf
      sendmailCommand
      ...
      emailConfigAuth
        <- getConfBool "EMAIL_AUTH" "email.auth" (Right True) conf
      let emailConfigSendmail = case sendmailCommand of
              Just cmd
                | (prg : args)
                  <- Text.splitOn " " cmd, not (Text.null prg) -> SendmailConfig
...
renderMsmtprc :: (MonadIO m, MonadLogger m)
              => EmailConfig
              -> Bool -- ^ Use TLS
...
renderMsmtprc cfg tls auth =
  let Mail.Address _ fromAddress = (cfg ^. from)
      dt =
        Aeson.object [ "host" Aeson..= (cfg ^. host)
                     , "port" Aeson..= (cfg ^. port)
...
sendEmail :: (MonadLogger m, MonadIO m, MonadThrow m)
          => EmailConfig
          -> Text -- ^ Email Address
          -> Text -- ^ Subject
...
sendEmail cfg toAddress subject parts = do
  ...
  let mail =
        Mail.addPart parts
                     ...
                     { Mail.mailHeaders = [ ("Subject", subject) ]
                     , Mail.mailTo      = [ to ]
                     }
  mbError <- liftIO . Ex.try $
    Mail.renderSendMailCustom (sendmailCfg ^. path)
...
sendHtmlEmail :: (MonadThrow m, MonadIO m, MonadLogger m)
              => EmailConfig
              -> Text -- ^ Address
              -> Text -- ^ Subject
...
sendHtmlEmail cfg toAddress subject body =
  ...
  in sendEmail cfg
               toAddress
               subject
               [ Mail.plainPart plainBody, Mail.htmlPart body ]

sendPlainEmail :: (MonadLogger m, MonadIO m, MonadThrow m)
               => EmailConfig
               -> Text -- ^ Address
               -> Text -- ^ Subject
               -> LText.Text -- ^ Body
