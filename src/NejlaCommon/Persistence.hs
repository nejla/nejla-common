...
data Privilege
  = Unprivileged -- ^ Operations that can be run by unprivileged
                 -- users
...
newtype App (st :: Type) (r :: Privilege) (l :: TransactionLevel) a =
  ...
  { unApp :: ReaderT (AppState st) IO a
    ...
    deriving ( Functor, Applicative, Monad, MonadFail, MonadIO, Ex.MonadThrow
             , Ex.MonadCatch, Ex.MonadMask, MonadBase IO, MonadUnliftIO )

instance MonadBaseControl IO (App st r l) where
  type StM (App st r l) a = a

  liftBaseWith f = App . ReaderT $ \st -> f (\(App m) -> runReaderT m st)
...
defaultSqlConfig =
  ...
  , sqlConfigRetryableErrors      = [ "40001" -- serialization_failure
                                    , "40P01" -- deadlock_detected
                                    ]
  ...
  res <- flip E.runSqlPool pool $ do
    ...
    liftIO $ Ex.catch (go con st $ conf L.^. numRetries) $
      \e -> case constraintViolation e of
          ...
          _ -> Ex.throwM $ DBError (Ex.toException e)
  after
  ...
  where
    ...
    go con st retries = do
      Ex.catch (liftIO $ runReaderT m st) $ \e -> do
        ...
        if Postgres.sqlState e `elem` (conf L.^. retryableErrors) && retries > 0
          then do
            -- Forget delayed IO actions before we restart the transaction
            atomicModifyIORef (appStateDelayedIO st) (const (return (), ()))
            delay
              <- randomRIO (conf L.^. retryMinDelay, conf L.^. retryMaxDelay)
            threadDelay delay
            go con st (retries - 1)
          else Ex.throwM e
...
-- monomorphic)
runApp' :: SingI l => SqlConfig -> ConnectionPool -> st -> App st p l a -> IO a
runApp' = runApp sing
...
withReadCommitted :: App st p 'ReadCommitted a -> App st p 'ReadCommitted a
...
instance ToJSON PersistError where
  toJSON (Conflict entity fields) =
    ...
    where
      fieldToJSON (fieldname, value') =
        object [ "field" ..= fieldname, "value" ..= value' ]
  toJSON (JSONDeserializationError e) =
    object [ "error" ..= "deserialization error", "message" .= e ]
  toJSON (ValueBound e) =
    object [ "error" ..= "out of bounds", "message" .= e ]
  toJSON (Policy e) = object [ "error" ..= "policy violation", "message" .= e ]
  toJSON (EntityNotFound tp v) =
    object [ "error" ..= "not found", "type" .= tp, "entity" .= v ]
  ...
  toJSON (DBError e) =
    object [ "error" ..= "database error", "value" ..= Text.pack (show e) ]
...
orL :: [SqlExpr (Value Bool)] -> SqlExpr (Value Bool)
orL [] = val False
...
andLMb :: [Maybe (SqlExpr (Value Bool))] -> SqlExpr (Value Bool)
...
arrayAgg'
  :: PersistField [a] => SqlExpr (Value (Maybe a)) -> SqlExpr (Value [a])
arrayAgg' x =
...
jsonFieldUUID v i =
  unsafeSqlBinOp "::" (jsonFieldText v i) (unsafeSqlValue "uuid")
...
sqlFormatTime :: SqlExpr (Value (Maybe UTCTime))
              -> SqlExpr (Value Text)
              -> SqlExpr (Value (Maybe Text))
sqlFormatTime time formatstring =
  unsafeSqlFunction "to_char" (time, formatstring)
...
getByNotFound
  :: (PersistEntity val, Show a, PersistEntityBackend val ~ SqlBackend)
  => Text
  -> a
  -> Unique val
...
mkForeignInstances ents = do
  ...
  concatForM defs $ \((f, t), pairss) -> case pairss of
      ...
      [ pairs' ] ->
        ...
        in [d|instance ForeignKey $(TH.conT $ TH.mkName f) $(TH.conT $
                                                           TH.mkName t) where
      ...
      _ -> do
        TH.reportWarning $
          concat [ "More than one possible Foreign instance for "
                 ...
                 , ": \n"
                 , List.intercalate "\n" . map ("      " <>) . for pairss $
                   \pairs' -> List.intercalate ", " $ for pairs' $
                   \(f', t') -> concat [ f', " -> ", t' ]
...
withPoolNoWait conf n f = do
  ...
  $logDebug $
    "Using connection string: \"" <> Text.decodeUtf8 connectionString <> "\""
...
withDBPool conInfo cons migr f = do
  logger <- askLoggerIO
  withPostgresqlPool (Postgres.postgreSQLConnectionString conInfo) cons $
    \pool -> do
...
-- established
runPoolRetry :: (MonadIO m, MonadUnliftIO m, Ex.MonadCatch m, MonadLogger m)
             => ConnectionPool
             -> ReaderT SqlBackend m a
             -> m a
runPoolRetry pool f = Ex.catchIOError (E.runSqlPool f pool) $ \e -> do
  liftIO $ threadDelay 1000000
  $logWarn $ "Could not connect to database, retrying ( "
